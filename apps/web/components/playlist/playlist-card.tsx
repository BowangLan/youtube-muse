...
import { GradientClassName } from "@/lib/intents";
...
export function PlaylistCard({ playlist, description, gradientClassName }: PlaylistCardProps) {
  const reduceMotion = useReducedMotion();
  const dispatch = usePlayerStore((state) => state.dispatch);
  const openIntentDetail = useV4AppStateStore((state) => state.openIntentDetail);
  const openPlaylistDetail = useV4AppStateStore(
    (state) => state.openPlaylistDetail
  );
  ...
  const isCustomPlaylist = customPlaylistIds.includes(playlist.id);

  // Get gradient - check for override first, then use intent's default
  const _gradientClassName =
    gradientOverrides[playlist.id] ?? gradientClassName;
  ...
  return (
    <div
      className={cn(
        ...
        gradientClassName &&
        (isActive ? `${_gradientClassName}-active` : _gradientClassName),
      )}
    >
      ...
      {/* Play/Pause Button */}
      <div className="absolute hidden sm:block bottom-3 right-5 z-30 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 trans duration-300">
        <button
          type="button"
          aria-label={
            isCurrentlyPlaying
